            ...
            {foreach $IMGs as $key=>$img}
                ...
                {if $ik <= $PH_INSTAGRAM_IMG_NUMBER}
                    <li class="instagram_item_img col-xs-4 col-sm-4 col-md-4">
                        <a class="ybc_instagram_fancy" href="{if $img.is_video}#ph_insta_video_{$key+1}{else}{$img.standard_resolution}{/if}" data-media-type="{if $img.is_video}video{else}image{/if}">
                        ...
                        {if $img.is_video}
                            <video controls style="display: none; padding: 0;" id="ph_insta_video_{$key+1}">
                                <source src="{$img.standard_resolution}" type="video/mp4">
