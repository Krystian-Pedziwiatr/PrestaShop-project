            ...
            {if isset($tc_config.YBC_TC_LAYOUT) && $tc_config.YBC_TC_LAYOUT == 'LAYOUT3'}
                <div class="current">
    				...
    				{foreach from=$currencies key=k item=f_currency}
    					...
    					{if $cookie->id_currency == $f_currency.id_currency}{$f_currency.name} <span>({$f_currency.iso_code})</span> {/if}
...
<div class="ybc_myaccout">
   {if $is_logged}
		<a class="logout" href="{$link->getPageLink('index', true, NULL, "mylogout")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Log me out' mod='blockuserinfo'}">
			<i class="fa fa-user" aria-hidden="true"></i>{l s='Log out' mod='blockuserinfo'}
		</a>        
	{else}
		<a class="login" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Log in to your customer account' mod='blockuserinfo'}">
			<i class="fa fa-user" aria-hidden="true"></i>{l s='Log in' mod='blockuserinfo'}
		</a>
	{/if} 
